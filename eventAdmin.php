...
<body>
                    ...
                    <tbody>
                        ...
                        foreach ($recent_rows as $row): ?>
                            <tr>
                                ...
                                <td><?= (isset($row['regno']) ? htmlspecialchars($row['regno']) : '') . ($row['type'] == 'Group' ? ' (Lead)' : '') ?>
                                </td>
                                <td><?= isset($row['year']) ? htmlspecialchars($row['year']) : '' ?></td>
                                <td><span class="status-badge status-active"><?= htmlspecialchars($row['type']) ?></span>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
    ...
    <script>
        $(document).ready(function () {
            ...
            // Global delegation for removing dynamic fields (fixes multiple listener issue)
            $(document).on('click', '.swal2-container .btn-remove-field', function () {
            ...
            $('.btn-edit-details').click(function () {
                ...
                // key fix: Parse as string to avoid type errors
                const eventRules = String(btn.data('rules') || '');
                const eventTopics = String(btn.data('topics') || '');

                // Helper function to create list input fields
                function createListInput(value = '', type) {
                    // key fix: escape double quotes for value attribute
                    const safeValue = value.replace(/"/g, '&quot;');
                    return `
                        <div class="input-group mb-2" style="display: flex; gap: 5px; margin-bottom: 5px;">
                            <input type="text" class="swal2-input ${type}-input" value="${safeValue}" placeholder="Enter ${type}" style="margin: 0 !important; width: 100%;">
                            <button type="button" class="btn-remove-field" style="background: #ef4444; color: white; border: none; border-radius: 4px; padding: 0 10px; cursor: pointer;">&times;</button>
                        </div>
                    `;
                }

                // key fix: strict check for empty string to avoid empty input
                const rulesArray = eventRules ? eventRules.split('\n') : [];
                const topicsArray = eventTopics ? eventTopics.split('\n') : [];

                let rulesHtml = '<div id="rules-container">';
                rulesArray.forEach(rule => { rulesHtml += createListInput(rule, 'rule'); });
                rulesHtml += '</div><button type="button" id="add-rule-btn" style="background: #10b981; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer; font-size: 0.8rem; margin-top: 5px;">+ Add Rule</button>';

                let topicsHtml = '<div id="topics-container">';
                topicsArray.forEach(topic => { topicsHtml += createListInput(topic, 'topic'); });
                topicsHtml += '</div><button type="button" id="add-topic-btn" style="background: #10b981; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer; font-size: 0.8rem; margin-top: 5px;">+ Add Topic</button>';
                ...
                Swal.fire({
                    ...
                    didOpen: () => {
                        ...
                        document.getElementById('add-rule-btn').addEventListener('click', () => {
                            ...
                            div.innerHTML = createListInput('', 'rule');
                        ...
                        document.getElementById('add-topic-btn').addEventListener('click', () => {
                            ...
                            const div = document.createElement('div');
                            div.innerHTML = createListInput('', 'topic');
                            container.appendChild(div.firstElementChild);
                        });
                        // Remove is handled by global delegation now
                    },
                    preConfirm: () => {
                        // Gather Rules
                        let rules = [];
