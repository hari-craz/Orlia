...
if (isset($_POST['Add_newuser'])) {
    try {
        ...
        // Generate Event Pass - Format: ORA1S01001
        // ORA = ORLIA prefix
        // 1 = Day number (1 or 2)
        // S = Solo event
        // 01 = Event table ID (2 digits)
        // 001 = Registration count (3 digits)
        ...
        $dayNumber = ($day == 'day1') ? '1' : '2';
        $typeCode = 'S'; // Solo
        $eventIdStr = str_pad($eventId, 2, '0', STR_PAD_LEFT);
        ...
        $eventPass = "ORA" . $dayNumber . $typeCode . $eventIdStr . $countStr;
...
if (isset($_POST['groupnewuser'])) {
    ...
    // Generate Event Pass (Team Leader) - Format: ORA1G01001
    // ORA = ORLIA prefix
    // 1 = Day number (1 or 2)
    // G = Group event
    // 01 = Event table ID (2 digits)
...
if (isset($_POST['fetch_event_keys'])) {
    // Filter out keys that are already assigned to a user
    $query = "SELECT event_key FROM events WHERE event_key NOT IN (SELECT userid FROM users) ORDER BY event_key ASC";
    ...
    echo json_encode(['status' => 200, 'data' => $keys]);
    exit;
}
...
if (isset($_POST['Addadmins'])) {
    try {
        ...
        // Permission Check
        if ($currentRole != '2') { // If not Super Admin
            if ($currentRole == '0') { // Co-Admin
                if ($newRole != '1') { // Can only add Event Admin
                    echo json_encode(['status' => 403, 'message' => 'Co-Admins can only add Event Admins.']);
                    exit;
                }
            } else {
                echo json_encode(['status' => 403, 'message' => 'You do not have permission to add admins.']);
                exit;
            }
        }
        ...
        $role = mysqli_real_escape_string($conn, $newRole);
...
if (isset($_POST['bulk_update_status'])) {
    ...
    if ($query && mysqli_query($conn, $query)) {
        echo json_encode(['status' => 200, 'message' => 'Status updated for selected scope']);
    } else {
        echo json_encode(['status' => 500, 'message' => 'Error updating status']);
    }
}

if (isset($_POST['bulk_delete_admins'])) {
    ...
    if (isset($_POST['ids']) && is_array($_POST['ids'])) {
        ...
        if (!empty($idsString)) {
            ...
            if (mysqli_query($conn, $query)) {
                echo json_encode(['status' => 200, 'message' => 'Selected admins deleted successfully']);
            } else {
                echo json_encode(['status' => 500, 'message' => 'Error deleting admins: ' . mysqli_error($conn)]);
            }
        } else {
            echo json_encode(['status' => 400, 'message' => 'No IDs provided']);
        }
    } else {
        echo json_encode(['status' => 400, 'message' => 'Invalid data format']);
    }
    exit;
}

if (isset($_POST['bulk_delete_events'])) {
    if (isset($_POST['ids']) && is_array($_POST['ids'])) {
        $ids = $_POST['ids'];
        // Sanitize IDs
        $sanitizedIds = array_map(function ($id) use ($conn) {
        ...
        if (!empty($idsString)) {
            $query = "DELETE FROM events WHERE id IN ($idsString)";
            if (mysqli_query($conn, $query)) {
                echo json_encode(['status' => 200, 'message' => 'Selected events deleted successfully']);
            } else {
                echo json_encode(['status' => 500, 'message' => 'Error deleting events: ' . mysqli_error($conn)]);
            }
        } else {
            echo json_encode(['status' => 400, 'message' => 'No IDs provided']);
        }
...
if (isset($_POST['add_new_event'])) {
    ...
    // Generate Key (PascalCase: capitalize each word, remove non-alphanumeric)
    $key = str_replace(' ', '', ucwords(strtolower(preg_replace('/[^a-zA-Z0-9\s]/', '', $name))));

    // Check availability
    $checkQ = "SELECT id FROM events WHERE event_key='$key'";
    if (mysqli_num_rows(mysqli_query($conn, $checkQ)) > 0) {
        echo json_encode(['status' => 409, 'message' => 'Event with this name/key already exists!']);
        exit;
    }
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, $allowedfileExtensions)) {
            ...
            if (move_uploaded_file($fileTmpPath, $dest_path)) {
                ...
                echo json_encode(['status' => 500, 'message' => 'Error moving the uploaded image.']);
                exit;
            }
        } else {
            echo json_encode(['status' => 400, 'message' => 'Invalid file format. Only JPG/PNG/WEBP allowed.']);
            exit;
        }
    }

    $query = "INSERT INTO events (event_name, event_key, event_type, day, event_venue, event_time, event_rules, event_topics, event_image, status) 
              VALUES ('$name', '$key', '$type', '$day', '$venue', '$time', '$rules', '$topics', '$imagePath', 1)";

    if (mysqli_query($conn, $query)) {
        echo json_encode(['status' => 200, 'message' => 'Event added successfully']);
    } else {
        echo json_encode(['status' => 500, 'message' => 'Error adding event: ' . mysqli_error($conn)]);
    }
}
if (isset($_POST['delete_event'])) {
    ...
    // $check = mysqli_query($conn, "SELECT event_image FROM events WHERE id='$id'");
    ...
    if (mysqli_query($conn, $query)) {
        ...
        echo json_encode(['status' => 500, 'message' => 'Error deleting event: ' . mysqli_error($conn)]);
    }
}
...
if (isset($_POST['update_event_details'])) {
    ...
    $query = "UPDATE events SET event_name='$name', event_type='$type', day='$day', event_venue='$venue', event_time='$time', event_rules='$rules', event_topics='$topics' WHERE id='$id'";
    if (mysqli_query($conn, $query)) {
...
if (isset($_POST['reset_ids'])) {
    // Security: Only Super Admin (role '2') can reset IDs
    if (session_status() === PHP_SESSION_NONE) {
        session_start();
    }

    if (!isset($_SESSION['role']) || $_SESSION['role'] != '2') {
        echo json_encode(['status' => 403, 'message' => 'Unauthorized. Only Super Admin can reset IDs.']);
        exit;
    }
    ...
    if (!empty($table)) {
        // Special handling for events table - sort by event_name, day, event_time before resetting IDs
        if ($type === 'events') {
            // Step 1: Get all events sorted by event_name ASC, day ASC, event_time ASC
            $selectQuery = "SELECT * FROM $table ORDER BY event_name ASC, day ASC, COALESCE(STR_TO_DATE(REPLACE(event_time, '.', ':'), '%l:%i %p'), STR_TO_DATE(event_time, '%l %p'), event_time) ASC";
            $result = mysqli_query($conn, $selectQuery);

            if (!$result) {
                echo json_encode(['status' => 500, 'message' => 'Failed to fetch events: ' . mysqli_error($conn)]);
                exit;
            }
            ...
            // Step 2: Delete all records temporarily
            $deleteQuery = "DELETE FROM $table";
            if (!mysqli_query($conn, $deleteQuery)) {
                echo json_encode(['status' => 500, 'message' => 'Failed to clear table: ' . mysqli_error($conn)]);
                exit;
            }
            ...
            if (!mysqli_query($conn, $resetQuery)) {
                echo json_encode(['status' => 500, 'message' => 'Failed to reset auto increment: ' . mysqli_error($conn)]);
                exit;
            }

            // Step 4: Re-insert events in sorted order (IDs will be auto-assigned sequentially)
            foreach ($events as $event) {
                ...
                if (!mysqli_query($conn, $insertQuery)) {
                    echo json_encode(['status' => 500, 'message' => 'Failed to re-insert event: ' . mysqli_error($conn)]);
                    exit;
                }
            }

            echo json_encode(['status' => 200, 'message' => 'Event IDs have been reset and sorted by name successfully.']);
        } else {
            // Standard reset logic for other tables
            $q1 = "SET @num := 0;";
...
if (isset($_GET['download_type'])) {
    ...
    switch ($type) {
        ...
        case 'videos':
            ...
            break;
        case 'events':
            $targetDir = $baseUploadDir . '/events';
            $zipPrefix = 'orlia_events_';
            break;
        case 'all':
        default:
            $targetDir = $baseUploadDir;
