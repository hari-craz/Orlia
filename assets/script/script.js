...
document.querySelectorAll('a[href^="#"], .hero-btn').forEach((anchor) => {
  anchor.addEventListener("click", function (e) {
    ...
    // If it's a regular link (not starting with #) and no data-target, allow default navigation
    if (href && !href.startsWith("#") && !this.getAttribute("data-target")) {
      return;
    }
    ...
    const targetId = href?.substring(1) || this.getAttribute("data-target");
...
function updateFooter() {
  ...
  if (footerElement) {
    ...
    footerElement.innerHTML = `&copy; ${currentYear} Syraa Groups. All Rights Reserved.`;
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Admin Sidebar Toggle (Enhanced for Mobile)
  ...
  if (sidebarToggle && adminSidebar) {
    ...
    // Close when clicking outside on mobile (backup)
    document.addEventListener("click", (e) => {
      if (
        window.innerWidth <= 991 &&
        adminSidebar.classList.contains("active")
      ) {
        // If clicked outside sidebar and toggle
        if (
