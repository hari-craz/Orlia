            ...
            <header class="admin-header">
                ...
                <div class="header-right">
                    <button id="bulkDeleteBtn"
                        style="padding: 10px 20px; font-size: 0.9rem; background-color: #ef4444; color: white; border: none; border-radius: 6px; cursor: pointer; box-shadow: none; margin-right: 15px; display: none; align-items: center; gap: 8px;">
            ...
            <div class="table-container">
                <table id="adminsTable" class="display" style="width:100%">
                    ...
                    <tbody>
                        ...
                        if ($currentRole == '0') { // If Co-Admin, hide Super Admins AND Co-Admins
                            $query .= " WHERE role = '1'";
                        }
        ...
        $(document).ready(function () {
            ...
            $('#bulkDeleteBtn').click(function () {
                ...
                Swal.fire({
                    ...
                    text: `You are about to delete ${selectedIds.length} admin(s). This cannot be undone!`,
                    icon: 'warning',
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    } catch (e) { console.error("Error parsing event keys"); }
                }
            });
            ...
            $('.add-admin').click(function () {
                let roleOptions = '';
                if (currentUserRole == '2') { // Super Admin
                    roleOptions = `
                        <option value="0">Co-Admin</option>
                        <option value="1">Event Admin</option>
                        <option value="2">Super Admin</option>
                     `;
                } else if (currentUserRole == '0') { // Co-Admin
                    roleOptions = `<option value="1">Event Admin</option>`;
                } else {
                    Swal.fire('Error', 'You do not have permission to add admins.', 'error');
                    return;
                }
                ...
                Swal.fire({
                    ...
                    didOpen: () => {
                        ...
                        function updateUsernameField() {
                            ...
                            if (selectedRole == '1') { // Event Admin
                                let options = '<option value="">Select Event Key</option>';
                                eventKeys.forEach(key => {
                                    options += `<option value="${key}">${key}</option>`;
                                });
                                usernameContainer.innerHTML = `<select id="swal-userid" class="swal-custom-select">${options}</select>`;
                            } else {
                                usernameContainer.innerHTML = `<input id="swal-userid" class="swal-custom-input" placeholder="Enter Username">`;
                            }
                        }

                        roleSelect.addEventListener('change', updateUsernameField);
                        // Trigger once to set initial state (e.g. if Co-Admin sees only Event Admin)
                        updateUsernameField();
                    },
                    preConfirm: () => {
                        ...
                        if (!userid) {
                            Swal.showValidationMessage('Please select/enter a username');
                            return false;
                        }
                        if (!password) {
